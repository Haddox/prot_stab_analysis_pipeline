...
# Define functions that perform the analysis
...
def report_model_ec50(dataset, model_parameters, fit_parameters, output_dir):
    model = (
...
def main():
    ...
    print("\nAnalyzing the datasets: {0}".format(', '.join(datasets)))
    ...
    if not os.path.isdir(output_dir):
        print("Making the direcotry: {0}".format(output_dir))
    ...
    for d, p in product(datasets, parameter_sets):
        print (d)
        my_fit_model = (d, p, fit_model(d, p), output_dir)
