...
def read_fastq_only_forward_read(filename, length_cds_to_parse=102):
    """
...
def ComputeCounts(fastq_files_and_output_file, forward_read_only=False):
    """
    ...
    for (i, fastq_file) in enumerate(fastq_files):
        if not forward_read_only:
            counts_per_file[i] = read_fastq(fastq_file)[1] # return the dictionary of counts
        else:
            counts_per_file[i] = read_fastq_only_forward_read(fastq_file)[1]
