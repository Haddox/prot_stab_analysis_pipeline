...
This pipeline also requires a program called [`PEAR`](https://sco.h-its.org/exelixis/web/software/pear/) to assemble paired-end deep-sequencing reads. Instructions for installing `PEAR` are provided on its website [`PEAR`](https://sco.h-its.org/exelixis/web/software/pear/).
